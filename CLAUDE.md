...
The setlist.fm search takes an optional `year` — a long-touring artist runs to hundreds of shows (Bad Bunny: 521, 27 pages of 20) and paging to yours is not viable. It **defaults to no filter**: results are newest-first, so page 1 is already the recent shows, and defaulting to the current year only hid history (Bad Bunny 521 → 48; Kendrick Lamar, last gig Dec 2025, → 0). Don't reintroduce that default. Selecting a year re-searches on `change`; there is no per-keystroke search, since each one is a throttled third-party call. Its response is **not client-cached** (`NO_CACHE_PREFIXES` in `cache/config.ts`) because it carries `configured`, a server capability that no mutation invalidates — cached, the tab stayed disabled for a day after the key was set.
