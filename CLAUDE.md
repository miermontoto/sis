...
Surfaces: a `concerts` section on the artist page (registered in `detail-layout.ts`), markers on the artist charts (`ChartEvent` gained `kind: 'concert'` + `href`), the global `/concerts` page (nav: Library), `concertsAttended`/`artistsSeenLive` on the profile card, and `LiveBadge` beside `Accolades` on the artist and track heroes. The track side is the reverse direction of the setlist match (`getTrackLiveConcerts` → `TrackDetail.liveConcerts`). The badge renders only when there *is* something to assert: its absence proves nothing, since a manually-logged concert has no setlist.

The setlist.fm search takes an optional `year` — a long-touring artist runs to hundreds of shows (Bad Bunny: 521, 27 pages of 20) and paging to yours is not viable. It searches on `change` (blur/Enter), never per keystroke: each search is a throttled third-party call. Its response is **not client-cached** (`NO_CACHE_PREFIXES` in `cache/config.ts`) because it carries `configured`, a server capability that no mutation invalidates — cached, the tab stayed disabled for a day after the key was set.
