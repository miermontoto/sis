...
import { resolvedEntityId, entityMergeJoin, userFilter, resolvedPlayJoins, albumNullFilter, playDuration, periodExpr, albumArtistId, albumArtistName } from './helpers.js';
...
export function getRawRanking(db: Db, entityType: EntityType, granularity: Granularity, weekStart: WeekStart, period: string, sort: Sort, limit: number, userId: number) {
  ...
  if (entityType === 'artist') {
    ...
    return db.all(sql`
      ...
      FROM (
        SELECT DISTINCT ${resolvedEntityId('artist')} as entity_id, lh.id as play_id, ${playDuration()} as duration_ms, ${pExpr} as p
  ...
  const groupCol = resolvedEntityId(entityType);
...
export function getChartFirstPeriods(db: Db, entityType: EntityType, granularity: Granularity, weekStart: WeekStart, currentPeriod: string, userId: number, entityIds: string[]): Record<string, string> {
  ...
  const rows = entityType === 'artist'
    ? db.all(sql`
      SELECT ${resolvedEntityId('artist')} as entity_id, min(${pExpr}) as first_period
      FROM listening_history lh
      JOIN tracks t ON t.spotify_id = lh.track_id
      JOIN track_artists ta ON ta.track_id = lh.track_id
    ...
    : db.all(sql`
      SELECT ${resolvedEntityId(entityType)} as entity_id, min(${pExpr}) as first_period
      FROM listening_history lh
      ${resolvedPlayJoins(entityType, userId)}
  ...
  if (entityType === 'artist') {
    ...
    ranked = db.all(sql`
      SELECT period, entity_id, rank FROM (
        ...
        FROM (
          SELECT DISTINCT ${pExpr} as period, ${resolvedEntityId('artist')} as entity_id, lh.id as play_id, ${playDuration()} as duration_ms
          FROM listening_history lh
          JOIN tracks t ON t.spotify_id = lh.track_id
          JOIN track_artists ta ON ta.track_id = lh.track_id
          ${entityMergeJoin('artist', userId)}
    ...
    ranked = db.all(sql`
      SELECT period, entity_id, rank FROM (
        SELECT ${pExpr} as period, ${resolvedEntityId(entityType)} as entity_id,
...
export function fetchEntityMetadata(db: Db, entityType: EntityType, ids: string[]): Map<string, EntityMeta> {
  ...
  if (entityType === 'track') {
    ...
    // un álbum lógico tiene su fila en `albums` como cualquier otro; lo único que no
    // tiene son temas propios, y de ahí el fallback de los helpers de artista
    const rows = db.all(sql`
      SELECT al.spotify_id as id, al.name, al.image_url,
             ${albumArtistName(sql`al.spotify_id`)} as artist_name,
             ${albumArtistId(sql`al.spotify_id`)} as artist_id
...
export function getEntityChartHistory(db: Db, entityType: EntityType, entityId: string, weekStart: WeekStart, sort: Sort, userId: number): ChartHistoryResponse {
  ...
  if (entityType === 'artist') {
    ...
    myData = db.all(sql`
      ...
      FROM (
        SELECT DISTINCT ${pExpr} as period, ${resolvedEntityId('artist')} as entity_id, lh.id as play_id, lh.played_at as played_at, ${playDuration()} as duration_ms
        FROM listening_history lh
        JOIN tracks t ON t.spotify_id = lh.track_id
        JOIN track_artists ta ON ta.track_id = lh.track_id
        ${mrJoin}
    ...
    allScores = db.all(sql`
      ...
      FROM (
        SELECT DISTINCT ${pExpr} as period, ${resolvedEntityId('artist')} as eid, lh.id as play_id, ${playDuration()} as duration_ms
        FROM listening_history lh
        JOIN tracks t ON t.spotify_id = lh.track_id
        JOIN track_artists ta ON ta.track_id = lh.track_id
        ${mrJoin}
    ...
    if (entityType === 'track') {
      groupCol = resolvedEntityId('track');
